...
For now, we put a placeholder for the `design` argument. Later on, we'll update it based on our actual experimental design.
...
In order to find differentially expressed genes (DEGs), we need to tell DESeq2 about our experimental design. We do this by updating the "design" attribute of the `dds.filt` object. We could have also provided `design` as an argument when we created the DESeq2 object above.
...
# each of the words after the ~ should correspond to columns in meta.df AKA colData(dds.filt)
# in our case, we only have antibiotic treatment, but another possible design formula 
# is something like ~ sex + age + timepoint
design(dds.filt) <- ~ abx
